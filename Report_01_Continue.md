...
| 优先级 | 任务 | 说明 |
|--------|------|------|
| **P0** | 数据统一管理迁移 | 将 Reforged 端的绑定/管理员/权限数据迁移到 Alpha 统一管理。目前数据存储分散在双端配置文件中，需要统一到 Alpha Redis，Reforged 仅作为执行器无状态运行 |
| P1 | Dashboard 前端完善 | 完善设置页面、添加用户管理界面 |
| P2 | 多服负载均衡 | 实现签到/物品发放的多服分流 |
| P3 | 更多游戏事件上报 | 死亡、成就、传送等事件 |

### 10.3 P0 任务详细说明：数据统一管理
